...
function Get-LatestSnapshotVersion {
    ...
    $ApiUrl = "https://api.github.com/repos/SIXOSN/komari-agent/releases?per_page=100"
    $ApiUrls = @($ApiUrl)
    if ($GitHubProxy -ne "") {
...
if ($InstallVersion -ne "") {
    ...
    $ApiUrl = "https://api.github.com/repos/SIXOSN/komari-agent/releases/latest"
    try {
...
$BinaryName = "komari-agent-windows-$arch.exe"
$DownloadUrl = if ($GitHubProxy) { "$GitHubProxy/https://github.com/SIXOSN/komari-agent/releases/download/$versionToInstall/$BinaryName" } else { "https://github.com/SIXOSN/komari-agent/releases/download/$versionToInstall/$BinaryName" }
